...
class eZEnhancedSelectionType extends eZDataType
{
    ...
    */
    function fetchClassAttributeHTTPInput( $http, $base, $classAttribute )
    {
        ...
        if ( $http->hasPostVariable( $base . "_ezenhancedselection_donotallowremoval_value_" . $classAttributeID ) )
        {
            $doNotAllowRemoval = true;
        }
    ...
    */
    function validateObjectAttributeOptionRemoval( $selectOptions, $contentObjectAttribute )
    {
        ...
        if ( $object->attribute( 'status' ) == EZ_CONTENT_OBJECT_STATUS_PUBLISHED && $classContent['do_not_allow_removal'] == 1 )
        {
            $publishedAttribute = eZContentObjectAttribute::fetch( $contentObjectAttribute->attribute( 'id' ), $object->attribute( 'current_version' ) );
            $publishedSelectOptions = $publishedAttribute->attribute( 'content' );

            $removedSelectOptions = array();

            //eZDebug::writeDebug( $publishedSelectOptions, 'published selected options' );

            foreach ( $publishedSelectOptions as $publishedSelectOption )
            {
                if ( !in_array( $publishedSelectOption, $selectOptions ) )
                {
                    $removedSelectOptions[] = $publishedSelectOption;
                }
            }

            $removedSelectOptionNames = array();

            foreach ( $classContent['options'] as $option )
            {
                if ( in_array( $option['identifier'], $removedSelectOptions ) )
                {
                    $removedSelectOptionNames[] = $option['name'];
                }
            }

            if ( count( $removedSelectOptionNames ) > 0 )
            {
                $contentObjectAttribute->setValidationError( 'You are not allowed to remove the previously selected option(s): ' . implode( ', ', $removedSelectOptionNames ) );
    ...
    */
    function validateObjectAttributeHTTPInput( $http, $base, $contentObjectAttribute )
    {
        ...
        if( $http->hasPostVariable( $base . '_ezenhancedselect_selected_array_' . $contentObjectAttribute->attribute( 'id' ) ) )
        {
            ...
            $intermediateState = eZEnhancedSelectionType::validateObjectAttributeOptionRemoval( $selectOptions, $contentObjectAttribute );
            if ( $intermediateState != EZ_INPUT_VALIDATOR_STATE_ACCEPTED )
            {
        ...
        {
            $intermediateState = eZEnhancedSelectionType::validateObjectAttributeOptionRemoval( array(), $contentObjectAttribute );
            if ( $intermediateState != EZ_INPUT_VALIDATOR_STATE_ACCEPTED )
            {
                return $intermediateState;
            }
    ...
    */
    function objectAttributeContent( $contentObjectAttribute )
    {
        ...
        if ( $idString != '' )
        {
            $selectedOptions = explode( '***', $idString );
        }
        else
        {
    ...
    */
	function classAttributeContent( $classAttribute )
	{
	    ...
	    return  array( 'options' => $optionArray,
	                   ...
	                   'do_not_allow_removal' => $classAttribute->attribute( 'data_int2' ),
